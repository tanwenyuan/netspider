 ...
 */
public class ZhiHuOper {

	private static Log log = LogFactory.getLog(ZhiHuOper.class);
	...
	public static void writeUserInfo(){
		...
		List<Map<String,Object>> allUserInfo = ZhiHuAllInfo.allElement();
	...
	public static void saveUserInfo(){
		...
		headers.put("Cookie", "*********************************");//从浏览器获取登录用户Cookie
		
		log.info("***********************************获取用户数据   Start**********************************************");
		Map<String,Object> eachUserInfo = null;
		String linkAddr = null;
		while(!AllUrlQueue.isEmpty()){
			linkAddr = AllUrlQueue.outUrl();
			log.info("***********************************获取用户URL :"+linkAddr);
			String reponseContent = HttpClientUtil.doGet(linkAddr, headers);
			eachUserInfo = eachUserInfo(reponseContent);
			log.info("***********************************获取用户数据 :"+eachUserInfo.toString());
			if(eachUserInfo != null)
				ZhiHuAllInfo.addInfo(eachUserInfo);
	...
	public static void requireLinks(int offset){
		...
		if(!StringUtils.isJSON(reponseContent))
			return;
		...
		try {
			...
			for (Element link : resultLinks) {
		        ...
		        if(matchFlag&&!AllUrlQueue.isContains(linkAddr))
		        	AllUrlQueue.addUrl(linkAddr);
	        }
	...
	public static void setNormalProperties(Properties normalProperties) {
		ZhiHuOper.normalProperties = normalProperties;
	}
